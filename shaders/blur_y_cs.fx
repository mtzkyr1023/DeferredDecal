...
[numthreads(1,LENGTH,1)]
void main(uint3 id : SV_DispatchThreadID, uint3 gtid : SV_GroupThreadID) {
	temporaryBuffer[gtid.y] = sourceTexture[id.xy];
	temporaryBuffer[min(gtid.y+ LENGTH, LENGTH * 2 - 1)] =
		sourceTexture[min(id.xy + uint2(0, LENGTH), (uint2)screenInfo.xy - uint2(1, 1))];
	...
	for (i = gtid.y; i < min(gtid.y + 8, LENGTH * 2 - 1); i++) {

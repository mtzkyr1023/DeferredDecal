...
void Player::update(float deltaTime) {
	...
	if (Input::Instance().Push(DIK_S)) {
		moveDirection += glm::vec3(-trs.forward().x, -trs.forward().y, -trs.forward().z) * 4.0f * deltaTime;
	}
	if (Input::Instance().Push(DIK_W)) {
		moveDirection += glm::vec3(trs.forward().x, trs.forward().y, trs.forward().z) * 4.0f * deltaTime;
	}
	if (Input::Instance().Push(DIK_A)) {
		moveDirection += glm::vec3(-trs.right().x, -trs.right().y, -trs.right().z) * 4.0f * deltaTime;
	}
